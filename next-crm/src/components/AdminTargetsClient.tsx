...
export default function AdminTargetsClient({ targets, individualTargets = [], auditLogs = [], users = [], user }: AdminTargetsProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (mode === 'global') {
        ...
        const selectedUserName = users.find(u => ((u as any).id || u.uid) === targetUser)?.name || 'Unknown';
                  ...
                  {mode === 'individual' && (
                    <div className="relative group">
                      ...
                      >
                        ...
                        {staffList.map(u => (
                        <option key={((u as any).id || u.uid)} value={((u as any).id || u.uid)}>{u.name} ({u.role})</option>))}
                      </select>
                    </div>
                  )}
                </div>
