...
export default function LeadModal({ isOpen, onClose, lead, user, leads = [] }: LeadModalProps) {
  ...
  const [internalLead, setInternalLead] = useState<Lead | null>(lead);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (internalLead) {
        const leadRef = doc(db, "leads", internalLead.id);
        if (user.role === 'staff') {
          ...
          if (isMasterDataChanged) {
            ...
            await updateDoc(leadRef, payloadToSave);
          ...
          try {
            ...
            if (!forecastSnap.empty) {
              const currentStatus = payloadToSave.status || internalLead?.status || 'Leads';
        ...
        if (existingData) {
          if (existingData.contact === formData.contact) {
            ...
            setLoading(false);
            setDuplicateConfirm({ isOpen: true, existingLead: { id: existingData.id, ...existingData }});
            return;
          }
        }
        ...
        // Add to history sub-collection so it shows up in LeadDetail
        const historyCollection = collection(db, "leads", docRef.id, "history");
        for (const entry of history) {
          await addDoc(historyCollection, entry);
        }
        
        // Final sanity check: Ensure Lead document has the correct funnelHistory array
        await updateDoc(docRef, { funnelHistory: history });
                  ...
                  <div className="mb-5 p-4 bg-indigo-50/50 border border-indigo-100 rounded-xl">
                    ...
                    {formData.dateClosed && (
                      <div className="mt-4 p-3 bg-emerald-50 border border-emerald-100 rounded-lg flex flex-col gap-3">
                        {(() => {
                          const previousWins = internalLead?.funnelHistory?.filter(h => h.stage === 'Close Win') || [];
