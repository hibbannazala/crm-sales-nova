...
export default function Dashboard({ leads, user, users, targets = [], individualTargets = [] }: DashboardProps) {
  ...
  const handleGlobalSync = async () => {
    if (!window.confirm("Sinkronisasi massal akan mencocokkan status Leads dengan Forecast untuk SEMUA data. Lanjutkan?")) return;
    
    setLoading(true);
    const toastId = toast.loading("Sedang menyinkronkan data...");
    
    try {
      const forecastsSnap = await getDocs(query(collection(db, "oiForecasts"), where("status", "==", "OPEN"), where("isDeleted", "==", false)));
      let updated = 0;
      const batch = writeBatch(db);

      console.log(`Checking ${forecastsSnap.size} open forecasts against ${leads.length} leads...`);

      for (const fDoc of forecastsSnap.docs) {
        const fData = fDoc.data();
        const fBrand = (fData.brandName || "").trim().toLowerCase();
        const fCategory = (fData.category || "").trim().toLowerCase();
        
        // Cari lead dengan pencocokan yang lebih fleksibel
        const lead = leads.find(l => (l.brandName || "").trim().toLowerCase() === fBrand && !l.isDeleted);
        
        if (lead && (lead.status === 'Close Win' || lead.status === 'Close Lost' || lead.status === 'Failed')) {
          // Cek kecocokan produk (HYPE, TNT, BASEMEN)
          const leadProducts = (lead.productOffered || []).map(p => p.trim().toLowerCase());
          
          // Forecast category biasanya "HYPE Campaign" atau "TNT Campaign"
          const isProductMatch = leadProducts.some(p => 
            fCategory.includes(p) || p.includes(fCategory.replace(' campaign', '').trim())
          );
          
          if (isProductMatch) {
            const newStatus = lead.status === 'Close Win' ? 'WIN' : 'LOSE';
            const fCampaign = Number(fData.campaignNumber || 1);
            
            // Cari campaign number di history lead yang paling baru/cocok
            const lWin = [...(lead.funnelHistory || [])].reverse().find(h => h.stage === 'Close Win');
            const lCampaign = Number(lWin?.campaignNumber || 1);

            if (fCampaign === lCampaign || newStatus === 'OPEN') {
              batch.update(doc(db, "oiForecasts", fDoc.id), {
                status: newStatus,
                updatedAt: new Date().toISOString()
              });
              updated++;
              console.log(`Match Found: ${fBrand} (${fCategory}) Camp-${fCampaign} will be updated to ${newStatus}`);
            }
          }
        }
      }
      ...
      await batch.commit();
      toast.success(`Berhasil menyinkronkan ${updated} data Forecast!`, { id: toastId });
    } catch (err) {
      toast.error("Gagal sinkronisasi massal", { id: toastId });
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const fixSuperImportData = async () => {
    if (!window.confirm("Jalankan script pintar untuk merapikan semua nama PIC di history?")) return;
    setLoading(true);
    const toastId = toast.loading("Memperbaiki data...");
    try {
      const leadsSnap = await getDocs(collection(db, "leads"));
      let updated = 0;
      const batch = writeBatch(db);
      
      // Map valid admin names in lowercase
      const validAdmins = admins.map(a => a.toLowerCase());
      
      for (const leadDoc of leadsSnap.docs) {
        const lead = leadDoc.data();
        let isModified = false;
        const newHistory = (lead.funnelHistory || []).map((h: any) => {
           if (h.by && typeof h.by === 'string') {
               // Check if the lowercase version of h.by is a valid admin name
               const lowerBy = h.by.toLowerCase();
               if (validAdmins.includes(lowerBy)) {
                   // Find the EXACT correct case from admins array
                   const correctAdminName = admins.find(a => a.toLowerCase() === lowerBy);
                   if (correctAdminName && h.by !== correctAdminName) {
                       isModified = true;
                       return { ...h, by: correctAdminName };
                   }
               }
           }
           return h;
        ...
        if (isModified) {
           batch.update(doc(db, "leads", leadDoc.id), { funnelHistory: newHistory });
           updated++;
        }
      }
      ...
      toast.success(`Selesai! ${updated} data leads berhasil dirapikan.`, { id: toastId });
    } catch (err) {
      toast.error("Gagal perbaiki data", { id: toastId });
      console.error(err);
    } finally {
      setLoading(false);
    }
                            ...
                            {isOverriddenByOther ? (
                              <div className="flex flex-col items-center gap-1.5" title={`Override oleh ${globalLatest.by}`}>
                                <span className={cn(
                                  ...
                                  getStatusColor(globalLatest.stage)
                                )}>
                                  {globalLatest.stage}
                                </span>
            ...
            <div className="space-y-4 overflow-y-auto flex-1 pr-2 custom-scrollbar z-10">
              {stagnantLeads.length === 0 ? (
                ...
                stagnantLeads.slice(0, 50).map((alert, idx) => (
                  <div key={idx} className={cn("p-4 rounded-2xl border transition hover:shadow-md cursor-pointer", alert.days >= 30 ? "bg-rose-50 border-rose-200" : "bg-amber-50 border-amber-200")} onClick={() => navigate(`/lead/${alert.lead.id}`)}>
