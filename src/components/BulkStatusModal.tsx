  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const lead of selectedLeads) {
        ...
        if (isCorrectionMode) {
          ...
          if (madeChanges) {
              ...
              const qSub = query(collection(db, "leads", lead.id, "history"), where("date", "==", wrongDate));
              const subSnap = await getDocs(qSub);
              subSnap.forEach(subDoc => {
                  batch.update(doc(db, "leads", lead.id, "history", subDoc.id), { date: date });
              });
          }
          continue;
        }
        ...
        cleanUndefined(funnelEntry);

        if (existingSelf) {
          const qSub = query(collection(db, "leads", lead.id, "history"), where("date", "==", date), where("stage", "==", status), where("by", "==", finalAuthor), limit(1));
          const subSnap = await getDocs(qSub);
          if (!subSnap.empty) {
            batch.set(doc(db, "leads", lead.id, "history", subSnap.docs[0].id), funnelEntry, { merge: true });
          }
        } else {
          // New sub-collection entry (using a random ID for batching)
          const newId = doc(collection(db, "leads", lead.id, "history")).id;
          batch.set(doc(db, "leads", lead.id, "history", newId), funnelEntry);
        }
      }
      ...
      if (snap.size > 48) {
        ...
        for (const d of toDelete) await deleteDoc(doc(db, "globalAuditLogs", d.id));
      }
      
      toast.success(`${selectedLeads.length} Lead berhasil di-update massal!`);
      onSuccess();
      onClose();
    } catch (error: any) {
      toast.error("Gagal Bulk Update: " + error.message);
    } finally {
      setLoading(false);
    }
  };
