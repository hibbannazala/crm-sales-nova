...
import OISummaryCards from './OISummaryCards';
import OIGrid from './OIGrid';
import OIMilestone from './OIMilestone';
...
export default function OIForecastPage({ leads, user, users = [] }: OIForecastPageProps) {
  ...
  const [forecasts, setForecasts] = useState<OIForecast[]>([]);
  const [targets, setTargets] = useState<OITarget[]>([]);
  ...
  useEffect(() => {
    ...
    // Failsafe if firebase hangs
    const timeoutId = setTimeout(() => {
      if (isMounted) setLoading(false);
    }, 2000);

    let forecastConstraints = [];
    let targetConstraints = [];

    // Optimized queries to drastically reduce Document Reads
    if (activeView === 'forecast') {
      forecastConstraints.push(where("monthYear", "==", selectedMonthYear));
      targetConstraints.push(where("monthYear", "==", selectedMonthYear));
    } else {
      forecastConstraints.push(where("monthYear", ">=", `${selectedYear}-01`));
      forecastConstraints.push(where("monthYear", "<=", `${selectedYear}-12`));
      targetConstraints.push(where("monthYear", ">=", `${selectedYear}-01`));
      targetConstraints.push(where("monthYear", "<=", `${selectedYear}-12`));
    }

    const qForecast = query(collection(db, "oiForecasts"), ...forecastConstraints);
    const unsubForecast = onSnapshot(qForecast, (snap) => {
      const data = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as OIForecast));
      if (isMounted) setForecasts(data);
    }, (err) => {
      console.error("Firebase err:", err);
      toast.error("Database connection delayed.");
    });

    const qTarget = query(collection(db, "oiTargets"), ...targetConstraints);
    const unsubTarget = onSnapshot(qTarget, (snap) => {
      const data = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as OITarget));
      if (isMounted) {
        setTargets(data);
        setLoading(false);
      }
  ...
  }, [activeView, selectedMonthYear, selectedYear]);
  ...
  useEffect(() => {
    ...
    const syncTimer = setTimeout(() => {
      forecasts.forEach(f => {
        ...
        const matchingWin = lead.funnelHistory?.find(h => h.stage === 'Close Win' && (h.campaignNumber || 1) === fCamp);
        ...
        if (needsUpdate) {
          ...
          updateDoc(doc(db, "oiForecasts", f.id), {
            ...updates,
  ...
  const getForecastPIC = (f: OIForecast) => {
    ...
    const winOrLost = lead.funnelHistory.slice().reverse().find(h => h.stage === 'Close Win' || h.stage === 'Close Lost');
