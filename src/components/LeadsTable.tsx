...
export default function LeadsTable({ leads, user, users, approvals }: LeadsTableProps) {
  ...
  const addAuditLog = async (action: string, details: string) => {
    try {
      const { collection, addDoc, query, orderBy, limit, getDocs, deleteDoc, doc: fDoc } = await import('firebase/firestore');
      ...
      // Maintain only 48 logs
      const q = query(collection(db, "globalAuditLogs"), orderBy("timestamp", "desc"));
      const snap = await getDocs(q);
      if (snap.size > 48) {
        const toDelete = snap.docs.slice(48);
        for (const d of toDelete) {
          await deleteDoc(fDoc(db, "globalAuditLogs", d.id));
        }
      }
    } catch (e) {
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          await updateDoc(fDoc(db, "leads", id), {
            isDeleted: true,
  ...
  const handleRestore = async (id: string) => {
    const lead = leads.find(l => l.id === id);
    try {
      const { updateDoc, doc: fDoc, deleteField } = await import('firebase/firestore');
      await updateDoc(fDoc(db, "leads", id), {
        isDeleted: false,
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          const forecastSnap = await getDocs(query(collection(db, "oiForecasts"), where("leadId", "==", id)));
          for (const fd of forecastSnap.docs) {
            await deleteDoc(doc(db, "oiForecasts", fd.id));
          }
      ...
      async () => {
        try {
          ...
          const promises = Array.from(selectedIds).map((id: string) => {
            if (viewMode === 'active') {
              return updateDoc(fDoc(db, "leads", id), {
                isDeleted: true,
                deletedAt: deletedAt.toISOString(),
                autoDeleteAt: autoDeleteAt.toISOString()
              });
              ...
              return (async () => {
                ...
                return deleteDoc(fDoc(db, "leads", id));
              })();
            }
          });
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          const { collection, getDocs, writeBatch, doc: fDoc } = await import('firebase/firestore');
          ...
          for (let i = 0; i < batchData.length; i += chunkSize) {
            ...
            for (const ld of slice) {
              ...
              if (newHist.length !== preCount) {
                ...
                batch.update(fDoc(db, "leads", ld.id), updatePayload);
                modifiedCount++;
              }
            }
            await batch.commit();
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          const { collection, query, where, getDocs, writeBatch, doc: fDoc } = await import('firebase/firestore');
          let deletedCount = 0;
          const CHUNK = 20;
          for (let i = 0; i < trashLeads.length; i += CHUNK) {
            ...
            for (const lead of slice) {
              const fcSnap = await getDocs(query(collection(db, "oiForecasts"), where("leadId", "==", lead.id)));
              fcSnap.docs.forEach(fd => batch.delete(fDoc(db, "oiForecasts", fd.id)));
              batch.delete(fDoc(db, "leads", lead.id));
              deletedCount++;
            }
            await batch.commit();
          }
