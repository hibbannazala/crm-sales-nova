...
export default function LeadDetail({ lead, user, users, onClose }: LeadDetailProps) {
  ...
  useEffect(() => {
    ...
    const qHistory = query(collection(db, "leads", lead.id, "history"), orderBy("timestamp", "desc"));
    const unsubHistory = onSnapshot(qHistory, (snapshot) => {
      const historyData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as (FunnelHistory & {id: string})[];
  ...
  const clearHistory = async () => {
    showConfirm(
      ...
      async () => {
        try {
          const { getDocs, deleteDoc, doc: fDoc } = await import('firebase/firestore');
          // Delete Sub-collection
          const snap = await getDocs(collection(db, "leads", lead.id, "history"));
          for (const d of snap.docs) {
            await deleteDoc(fDoc(db, "leads", lead.id, "history", d.id));
          }
          
          // Clean up related Forecasts
          const forecastSnap = await getDocs(query(collection(db, "oiForecasts"), where("leadId", "==", lead.id)));
          for (const fd of forecastSnap.docs) {
            await deleteDoc(fDoc(db, "oiForecasts", fd.id));
          }
          
          await updateDoc(doc(db, "leads", lead.id), {
            notes: [],
            funnelHistory: [], // Compatibility
            dateChated: null,
            dateResponsed: null,
            dateSetMeeting: null,
            dateClosed: null,
  ...
  const deleteNote = async (originalNoteObj: any) => {
    try {
      ...
      const realIndex = lead.notes.findIndex(n => n.timestamp === originalNoteObj.timestamp && n.text === originalNoteObj.text);
      if (realIndex !== -1) {
        const newNotes = [...lead.notes];
        newNotes.splice(realIndex, 1);
  ...
  const handleAddNote = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, "leads", lead.id), { notes: newNotes });
      setNewNoteText('');
  ...
  const deleteHistoryItem = async (historyId: string, item: FunnelHistory) => {
    try {
      const { deleteDoc, doc: fDoc, getDocs } = await import('firebase/firestore');
      await deleteDoc(fDoc(db, "leads", lead.id, "history", historyId));

      // If Close Win is deleted, remove related forecast
      ...
      // Sync summary dates and MAIN STATUS
      const updates: any = {};
      ...
      if (remainingOfStage.length === 0) {
        if (item.stage === 'Chated') updates.dateChated = null;
        else if (item.stage === 'Responsed') updates.dateResponsed = null;
        else if (item.stage === 'Set Meeting') updates.dateSetMeeting = null;
        else if (item.stage.includes('Close')) updates.dateClosed = null;
      }

      // AUTO-SYNC STATUS & FULL HISTORY ARRAY
      updates.funnelHistory = remainingHistory.map(({ id, ...rest }: any) => rest); // Clean up IDs before saving to array

      if (remainingHistory.length > 0) {
        // Sort by timestamp or date to find the absolute latest
        ...
        // If the latest is now NOT a Close Win, reset dealValue
        if (latest.stage !== 'Close Win') {
          updates.dealValue = 0;
        } else {
          updates.dealValue = latest.dealValue || 0;
        }
      } else {
        // If no history left at all (shouldn't happen with Input Data), default to Leads
  ...
  const saveHistoryEdit = async () => {
    ...
    try {
      const { updateDoc: updateSub, doc: fDoc } = await import('firebase/firestore');
      await updateSub(fDoc(db, "leads", lead.id, "history", editingHistoryId), editHistoryData as any);
      ...
      await updateDoc(doc(db, "leads", lead.id), {
        funnelHistory: updatedArray,
        status: isLatest ? (editHistoryData.stage as any) : lead.status,
        dealValue: (isLatest && editHistoryData.stage === 'Close Win') ? editHistoryData.dealValue : lead.dealValue
      });
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="px-8 border-b border-slate-100 flex items-center gap-8 shrink-0">
            ...
            <TabButton active={activeTab === 'notes'} onClick={() => setActiveTab('notes')} label={`Notes (${lead.notes?.length || 0})`} />
          ...
          <div className="flex-1 overflow-y-auto p-8">
            <AnimatePresence mode="wait">
              {activeTab === 'overview' && (
                ...
                >
                  <div className="grid grid-cols-3 gap-6">
                    <StatCard label="Total Notes" value={lead.notes?.length || 0} icon={<MessageSquare className="w-5 h-5 text-amber-500" />} />
                    <StatCard label="Funnel Steps" value={(fullHistory.length > 0 ? fullHistory : (lead.funnelHistory || [])).length || 0} icon={<History className="w-5 h-5 text-indigo-500" />} />
                  ...
                  <div className="grid grid-cols-2 gap-8">
                    <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm">
                      ...
                      <div className="space-y-4">
                        {(() => {
                          const history = fullHistory.length > 0 ? fullHistory : (lead.funnelHistory || []);
              ...
              {activeTab === 'notes' && (() => {
                ...
                if (lead.actionPlan && !allNotes.some(n => (n as any).isLegacyActionPlan)) {
                  allNotes.unshift({
                    ...
                    author: lead.funnelHistory[0]?.by || 'Sistem (Awal)',
              ...
              {activeTab === 'history' && (
                ...
                >
                  {(fullHistory.length > 0 ? fullHistory : (lead.funnelHistory || [])).map((h: any, i) => {
